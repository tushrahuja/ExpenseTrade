...
def goals_page(cur, conn):
    ...
    view_tab, add_tab = st.tabs(["View Goals", "Set New Goal"])
    ...
    with view_tab:
        ...
        '''
        try:
            goals = cur.execute(goals_query, (st.session_state["username"],)).fetchall()
            ...
            if submitted:
                if goal_amount <= 0:
                    ...
                    try:
                        ...
                        VALUES (?, ?, ?)
                        '''
                        cur.execute(insert_query, (st.session_state["username"], goal_amount, goal_description))
                        conn.commit()
...
# Add the new "Goals" tab to the sidebar navigation


# In the main content, add functionality for Goals

# Establish database connection for goals
goals_conn = sqlite3.connect('goals.db', check_same_thread=False)
goals_cur = goals_conn.cursor()

goals_page(goals_cur, goals_conn)
